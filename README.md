...
> **Note:** `sync_runs` only tracks *whether* the last run is stuck/stale (used by `getSyncSnapshot`'s crash-recovery check) — it doesn't hold per-run counts. Progress and totals always come from a fresh, live query of `drive_files` (`fileCounts`, returned by `/sync/events`), which represents all-time totals across every folder and run for that user.
