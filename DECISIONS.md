...
- Switched the Claude Code Review workflow from OAuth token auth to a plain Anthropic API key while debugging a review failure.
- Added a CI step to build the project and fail the workflow if committed dist/ drifts from a fresh build, and updated the review prompt to tell Claude it can't run tests/tsc itself (read-only tools) so it should judge test coverage by reading test files and say explicitly it didn't run the suite.
