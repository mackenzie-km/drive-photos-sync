...
`finishRun`'s terminal push also queries `getFileCounts` directly rather than tracking its own upload counts — `finishRun` takes only `(userId, runId, status)`, no count parameters at all, because `startSync`'s fire-and-forget `.catch()` calls `finishRun(userId, runId, "failed")` when `runSync` throws mid-sync (e.g. an expired `driveAccessToken` partway through discovery), and at that point there's no reliable count of what was actually discovered/uploaded before the failure to pass in anyway.
...
### Schema changes beyond initial creation go through migrations, not hand-edited `ALTER` statements

`initDb()`'s `CREATE TABLE IF NOT EXISTS` blocks define the desired shape for a **brand-new** database only — they're a no-op against a table that already exists, so editing a column list there does nothing to an already-deployed database (Render's, for instance). Changes that need to reach existing databases (dropping/renaming a column, backfilling data) go in the `migrations` array in `db.ts` instead: each entry is `{ name, sql }`, run once and recorded in a `schema_migrations` table, checked by `name` before running. To add one, append a new entry — never edit or reorder past entries once they've shipped, since a changed `name` would make `runMigrations` think it's unapplied and rerun it. `runMigrations()` runs automatically at the end of `initDb()`, before the server starts accepting requests.
...
- **`sync_runs` and `drive_files` are not linked by foreign key**. `sync_runs` only holds `id`/`user_id`/`status`/`started_at` — enough for `getSyncSnapshot`'s stale/stuck-run detection — not per-run counts; individual file attribution across runs is not tracked. Per-run `total`/`uploaded`/`skipped`/`failed`/`completed_at` columns existed at one point but were dropped via migration `0001_drop_sync_runs_unused_columns` (see below): nothing ever read them back (`fileCounts`, a live query of `drive_files`, is what's actually shown to users), so don't reintroduce them without a concrete reader.
